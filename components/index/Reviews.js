...
const Reviews = () => {
  ...
  useEffect(() => {
    ...
    FB.XFBML.parse(ref?.current, function () {
      ...
      if (ref?.current?.clientHeight < 200) {setIframeError(true);}
     //When the iframe is rendered we check if its height is greater than 200px. if so, we are sure that the iframe has been rendered correctly
    });
